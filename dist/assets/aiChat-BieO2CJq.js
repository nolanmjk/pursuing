...
当前日期：2026年5月。用户是甘肃高三毕业生。`;async function l(e,t={}){let{max_tokens:n=600}=t;try{let t=await fetch(`${s}/chat/completions`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({model:`deepseek-chat`,messages:[{role:`system`,content:c},...e],temperature:.7,max_tokens:n})});return t.ok&&(await t.json()).choices?.[0]?.message?.content||null}catch{return null}}function u({userScore:e,userRank:t,userSubject:n}){if(!e&&!t)return``;let r=[];return e&&r.push(`分数：${e}分`),t&&r.push(`位次：${t.toLocaleString()}名`),n&&r.push(`科类：${n}`),r.length>0?`\n\n（用户信息：${r.join(`，`)}。如果用户问及分数相关的问题，请结合这些信息给出个性化建议。）`:``}export{u as n,o as r,l as t};